...
export function TodayStream() {
  ...
  return (
    <div className="mobile-page-shell" style={{ display: "flex", flexDirection: "column", gap: "16px", padding: "20px" }} data-testid="field-agent-today">
      <section aria-labelledby="today-shift-heading"><h1 id="today-shift-heading" className="sr-only">Today&apos;s field shift</h1><ShiftCard /></section>
      ...
      <FieldChatSurface />
      {scheduled.length > 0 &&      <section aria-labelledby="up-next-heading"><div id="up-next-heading" className="label-micro jobs-section-label" style={{ marginBottom: "10px" }}>UP NEXT · {scheduled.length}</div>{scheduled.slice(0, 3).map((job) => <CompactJobRow key={job.id} job={job} onOpen={openJob} onClockIn={(id) => startClockOn(id, currentStaffId)} />)}</section>}
