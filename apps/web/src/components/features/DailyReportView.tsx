...
function VoiceButton({ onText, label }: { onText: (text: string) => void; label: string }) {
  ...
  return (
    <button
      ...
      title={supported ? `Dictate ${label}` : "Voice input is not supported on this browser"}
      className={`inline-flex items-center justify-center w-9 h-9 rounded-lg border transition haptic ${
        listening ? "bg-accent/20 text-accent border-accent/40" : "bg-white/[0.04] text-slate-400 border-white/[0.08]"
      } disabled:opacity-35`}
    >
      <Mic size={15} />
    </button>
      ...
      <div className="grid grid-cols-2 gap-2">
        {options.map((option) => (
          <button
            ...
            className="text-left min-h-[56px] rounded-xl border border-white/[0.08] bg-white/[0.04] px-3 py-2.5 text-white text-sm font-semibold active:scale-[0.97] transition"
          >
            <span className="block truncate">{option.description}</span>
            <span className="block text-[11px] text-slate-500 mt-0.5">per {option.unit}{option.rate ? ` · $${option.rate}` : ""}</span>
          </button>
        ))}
      </div>
      <div className="mt-4 pt-4 border-t border-white/[0.08]">
        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Custom item</label>
        <div className="flex gap-2">
          <input
            value={custom}
            onChange={(event) => setCustom(event.target.value)}
            placeholder="e.g. isolation valve"
            className="min-w-0 flex-1 min-h-[48px] app-input border rounded-xl px-3 text-sm text-white placeholder-slate-600"
          />
          <button
            type="button"
            disabled={!custom.trim()}
            onClick={() => { onAdd({ id: `custom-${crypto.randomUUID()}`, description: custom.trim(), unit: "ea", rate: 0 }); setCustom(""); onClose(); }}
            className="min-h-[48px] px-4 rounded-xl bg-accent text-white font-semibold text-sm disabled:opacity-35"
          >Add</button>
        </div>
      </div>
    </BottomSheet>
...
function DailyReportForm({ job }: { job: Job }) {
  const { members, quotes, dispatch, postMessage, setView } = usePlumbTrackCtx();
  ...
  const saveReport = (submit: boolean) => {
    ...
    return (
      <div className="p-3">
        <GlassCard className="text-center">
          ...
          <p className="font-semibold text-white">Quick service job</p>
          <p className="text-xs text-slate-500 mt-1">Use Clock, Photos, and Sign-off for this callout. A full daily report is reserved for project work.</p>
          <button type="button" onClick={() => setView("job")} className="mt-4 w-full min-h-[48px] rounded-xl bg-accent text-white font-semibold text-sm">Back to job</button>
        </GlassCard>
      </div>
    );
  }
    ...
    return (
      <div className="p-3 space-y-2">
        <GlassCard className="text-center">
          ...
          <p className="font-semibold text-white">Daily Report Submitted</p>
          <p className="text-xs text-slate-500 mt-0.5">{report.date} · posted to #field-updates</p>
          <button type="button" onClick={() => setView("job")} className="mt-4 w-full min-h-[48px] rounded-xl surface-card text-slate-300 text-sm font-semibold border border-white/[0.08]">Back to job</button>
        </GlassCard>
      </div>
    );
  }
    ...
    <div className="p-3 pb-28 space-y-2">
      <GlassCard className="!p-3">
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="text-sm font-semibold text-white truncate">{job.client}</p>
            <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5"><MapPin size={11} /> {job.address}</p>
          </div>
          <span className="shrink-0 text-[10px] font-bold uppercase text-slate-500 border border-white/[0.08] rounded-full px-2 py-1">Project mode</span>
        </div>
      </GlassCard>
      ...
      <GlassCard>
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {crewIds.length === 0 && <span className="text-xs text-slate-500">No one clocked on yet. Crew appears automatically from this job’s time entries.</span>}
        </div>
      </GlassCard>
      ...
      <GlassCard>
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 relative">
            <input value={weather} onChange={(event) => { setWeatherEdited(true); setWeather(event.target.value); }} placeholder={weatherState === "loading" ? "Reading local weather…" : "Weather unavailable — tap to add"} className="w-full min-h-[44px] app-input border rounded-xl px-3 text-sm text-white placeholder-slate-600" />
            {weatherState === "loading" && <LoaderCircle size={14} className="absolute right-3 top-3.5 text-slate-500 animate-spin" />}
          </div>
          <span className="text-[10px] text-slate-500 whitespace-nowrap">GPS / live</span>
        </div>
      </GlassCard>

      <GlassCard>
        <FieldLabel icon={<Wrench size={13} />} voice={<VoiceButton label="work completed" onText={(text) => updateText(setWorkDone, text)} />}>
          Work completed
        </FieldLabel>
        <div className="flex flex-wrap gap-1.5 mb-2">
          {WORK_CHIPS.map((chip) => {
            const active = selectedWork.includes(chip);
            return <button key={chip} type="button" onClick={() => setSelectedWork((current) => active ? current.filter((item) => item !== chip) : [...current, chip])} className={`min-h-[36px] rounded-full border px-3 text-xs font-semibold transition active:scale-[0.97] ${active ? "bg-accent/15 border-accent/35 text-accent" : "bg-white/[0.03] border-white/[0.08] text-slate-400"}`}>{active ? "✓ " : "+ "}{chip}</button>;
          })}
        </div>
        <textarea value={workDone} onChange={(event) => setWorkDone(event.target.value)} placeholder="Add a short note or use the mic" rows={2} className="w-full app-input border rounded-xl px-3 py-2.5 text-sm text-white placeholder-slate-600 resize-none" />
        {todayLogs.filter((entry) => entry.kind === "production").map((entry) => <p key={entry.id} className="text-xs text-slate-500 mt-2">• {entry.description}{entry.quantity ? ` · ${entry.quantity}${entry.unit ?? ""}` : ""}</p>)}
      </GlassCard>
      ...
      <GlassCard>
        ...
        {materials.length === 0 ? <p className="text-xs text-slate-600 py-2">No materials logged. Add quoted items or choose a common field item.</p> : <div className="space-y-1.5">
          {materials.map((item) => <div key={item.id} className="flex items-center gap-2 min-h-[40px] rounded-lg bg-white/[0.03] border border-white/[0.06] px-2.5"><span className="flex-1 min-w-0 text-sm text-slate-300 truncate">{item.description}</span><button type="button" onClick={() => setMaterials((current) => current.map((entry) => entry.id === item.id ? { ...entry, qty: Math.max(1, entry.qty - 1) } : entry))} className="w-7 h-7 rounded-md bg-white/[0.06] text-slate-300">−</button><span className="w-5 text-center text-xs font-mono text-white">{item.qty}</span><button type="button" onClick={() => setMaterials((current) => current.map((entry) => entry.id === item.id ? { ...entry, qty: entry.qty + 1 } : entry))} className="w-7 h-7 rounded-md bg-white/[0.06] text-slate-300">+</button><button type="button" onClick={() => setMaterials((current) => current.filter((entry) => entry.id !== item.id))} aria-label={`Remove ${item.description}`} className="w-7 h-7 rounded-md text-slate-500 hover:text-red-400"><X size={14} /></button></div>)}
          {materialTotal > 0 && <p className="text-[11px] text-slate-500 text-right pt-1">Quoted value logged · ${materialTotal.toFixed(2)}</p>}
        </div>}
      </GlassCard>

      <GlassCard>
        <FieldLabel icon={<AlertTriangle size={13} />} voice={<VoiceButton label="delays and issues" onText={(text) => updateText(setDelays, text)} />}>Issues / delays <span className="font-normal normal-case tracking-normal text-slate-600">optional</span></FieldLabel>
        <textarea value={delays} onChange={(event) => setDelays(event.target.value)} placeholder="Only add something if it needs follow-up" rows={2} className="w-full app-input border rounded-xl px-3 py-2.5 text-sm text-white placeholder-slate-600 resize-none" />
      </GlassCard>

      <GlassCard className="!p-3">
        <div className="flex items-center justify-between gap-2 text-xs text-slate-500"><span>Visitors are optional for this log</span><button type="button" onClick={() => setVisitors((current) => current ? "" : "Visitor recorded") } className={`min-h-[36px] px-3 rounded-full border font-semibold ${visitors ? "bg-accent/10 border-accent/25 text-accent" : "border-white/[0.08] text-slate-400"}`}>{visitors ? "Visitor added" : "Add visitor"}</button></div>
        {visitors && <input autoFocus value={visitors} onChange={(event) => setVisitors(event.target.value)} placeholder="Name or company" className="mt-2 w-full min-h-[44px] app-input border rounded-xl px-3 text-sm text-white placeholder-slate-600" />}
      </GlassCard>
