    ...
    <div className="p-3 space-y-2">
      <GlassCard>
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-bold text-slate-300 uppercase tracking-wider">Sync centre</p>
            <p className="text-[11px] text-slate-500 mt-1">Field work is saved locally first and delivered in the background.</p>
          </div>
          <Cloud size={18} className={syncStatus.failed > 0 ? "text-red-300" : syncStatus.pending > 0 ? "text-amber-300" : "text-accent"} />
        </div>
        <div className="mt-3 rounded-xl bg-white/[0.04] border border-white/[0.06] px-3 py-2.5 flex items-center justify-between gap-3">
          <span className="text-xs text-slate-400">{syncStatus.label}</span>
          <span className="text-[10px] text-slate-600">{operations.length} queued</span>
        </div>
      </GlassCard>
      ...
      {operations.length === 0 ? (
        <GlassCard>
          <div className="py-8 text-center">
            ...
            <p className="text-sm font-semibold text-white">Everything is up to date</p>
            <p className="text-xs text-slate-500 mt-1">No field updates need delivery.</p>
          </div>
        </GlassCard>
      ) : (
          ...
          return (
            <GlassCard key={operation.id} className="!p-3">
              <div className="flex items-start gap-3">
                <span className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${failed ? "bg-red-500/15 text-red-300" : operation.kind === "photo-upload" ? "bg-accent/15 text-accent" : "bg-white/[0.06] text-slate-300"}`}>
                  {failed ? <AlertTriangle size={16} /> : operation.kind === "photo-upload" ? <Upload size={16} /> : <Cloud size={16} />}
                </span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-sm font-semibold text-white truncate">{operationLabel(operation)}</p>
                    <span className={`text-[9px] uppercase tracking-wider font-bold shrink-0 ${failed ? "text-red-300" : operation.status === "processing" ? "text-accent" : "text-amber-300"}`}>
                      {failed ? "Needs attention" : operation.status}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 mt-1 truncate">{operationDetail(operation)}</p>
                  <p className={`text-[10px] mt-1 ${failed ? "text-red-300" : "text-slate-600"}`}>{failed ? operation.lastError : retryTime(operation)}</p>
                </div>
              </div>
              {failed && (
                <div className="flex gap-2 mt-3 pt-3 border-t border-white/[0.06]">
                  ...
                  <button type="button" onClick={() => { void discardFailedSync(); }} className="min-h-[44px] px-3 rounded-xl bg-white/[0.04] text-slate-400 text-xs font-semibold border border-white/[0.08]" aria-label="Dismiss failed updates"><Trash2 size={13} /></button>
                </div>
              )}
            </GlassCard>
          );
      ...
      {syncStatus.failed > 0 && operations.length > 1 && (
        <button type="button" onClick={() => { void retryFailedSync(); }} className="w-full min-h-[48px] rounded-xl bg-white/[0.04] border border-white/[0.08] text-slate-300 text-xs font-semibold">Retry all failed updates</button>
      )}
    </div>
  );
}
