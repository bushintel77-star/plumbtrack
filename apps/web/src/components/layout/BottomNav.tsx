import { Wrench, MessageSquare, ClipboardList, Ellipsis } from "lucide-react";
import type { Tab } from "@/types";

interface BottomNavProps {
  activeTab: Tab;
  onTabChange: (tab: Tab) => void;
  /** Opens the More sheet (Quotes / Documents / Settings). */
  onMorePress: () => void;
  /** Whether a More destination is the currently active tab. Defaults to
   *  highlighting More whenever one of its destinations is on screen. */
  moreActive?: boolean;
  unreadCount?: number;
}

/** Weekly-touch destinations that live behind the More tab. */
const MORE_TABS: Tab[] = ["dashboard", "documents", "settings"];

export function BottomNav({
  activeTab,
  onTabChange,
  onMorePress,
  moreActive = MORE_TABS.includes(activeTab),
  unreadCount = 0,
}: BottomNavProps) {
  // High-frequency destinations get permanent tabs; everything else is one
  // tap away in the More sheet.
  const primaryTabs: { key: Tab; icon: typeof Wrench; label: string }[] = [
    { key: "jobs", icon: Wrench, label: "Jobs" },
    { key: "messages", icon: MessageSquare, label: "Messages" },
    { key: "quotes", icon: ClipboardList, label: "Quotes" },
  ];

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-20 app-bottom-nav" aria-label="Primary">
      {/* Frosted glass bar — safe-area padding keeps tabs clear of the home indicator */}
      <div className="app-nav backdrop-blur-xl border-t pb-[env(safe-area-inset-bottom)]">
        <div className="flex">
          {primaryTabs.map(({ key, icon: Icon, label }) => {
            const active = activeTab === key;
            return (
              <button
                key={key}
                type="button"
                onClick={() => onTabChange(key)}
                className={`flex-1 flex flex-col items-center gap-0.5 py-2.5 min-h-[56px] haptic relative ${                    active ? "text-accent" : "text-ink-low"
                }`}
                aria-label={label}
                aria-current={active ? "page" : undefined}
              >
                {/* Active indicator pill — reserved glow surface (chrome-200) */}
                {active && (
                  <span
                    className="absolute -top-px left-1/2 -translate-x-1/2 w-8 h-[3px] rounded-full animate-fade-in"
                    style={{
                      backgroundColor: "var(--chrome-200)",
                      boxShadow: "0 0 12px 2px var(--chrome-400), 0 1px 6px var(--chrome-400)",
                    }}
                  />
                )}
                <span className="relative">
                  <Icon size={20} strokeWidth={active ? 2.5 : 1.5} />
                  {key === "messages" && unreadCount > 0 && (
                    <span className="absolute -top-1.5 -right-2.5 min-w-[16px] h-4 px-1 rounded-full bg-accent text-on-accent text-[9px] font-bold flex items-center justify-center leading-none animate-scale-in">
                      {unreadCount > 9 ? "9+" : unreadCount}
                    </span>
                  )}
                </span>
                <span className={`text-[10px] font-semibold transition-colors ${active ? "text-accent" : "text-ink-low"}`}>
                  {label}
                </span>
              </button>
            );
          })}

          {/* More — sheet entry point for weekly-touch destinations */}
          <button
            type="button"
            onClick={onMorePress}
            className={`flex-1 flex flex-col items-center gap-0.5 py-2.5 min-h-[56px] haptic relative ${
              moreActive ? "text-accent" : "text-ink-low"
            }`}
            aria-label="More"
            aria-haspopup="dialog"
            aria-current={moreActive ? "page" : undefined}
          >
            {moreActive && (
              <span
                className="absolute -top-px left-1/2 -translate-x-1/2 w-8 h-[3px] rounded-full animate-fade-in"
                style={{
                  backgroundColor: "var(--chrome-200)",
                  boxShadow:
                    "0 0 12px 2px var(--chrome-400), 0 1px 6px var(--chrome-400)",
                }}
              />
            )}
            <Ellipsis size={20} strokeWidth={moreActive ? 2.5 : 1.5} />
            <span
              className={`text-[10px] font-semibold transition-colors ${moreActive ? "text-accent" : "text-ink-low"}`}
            >
              More
            </span>
          </button>
        </div>
      </div>
    </nav>
  );
}