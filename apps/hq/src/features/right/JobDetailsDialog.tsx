...
export function JobDetailsDialog() {
  ...
  return (
    ...
    <motion.aside
      ...
      <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto p-3"><div className="mb-3 grid grid-cols-3 gap-2"><div className="rounded-lg border border-line/80 bg-recess/60 p-2"><div className="label-mono text-2xs text-ink-low">STATUS</div><div className="mt-1 text-xs font-semibold capitalize">{job.status.replace("_", " ")}</div></div><div className="rounded-lg border border-line/80 bg-recess/60 p-2"><div className="label-mono text-2xs text-ink-low">PRIORITY</div><div className="mt-1 text-xs font-semibold capitalize">{job.priority}</div></div><div className="rounded-lg border border-line/80 bg-recess/60 p-2"><div className="label-mono text-2xs text-ink-low">WINDOW</div><div className="mt-1 label-mono tnum text-xs font-semibold">{blockLabel(job.startBlock)}</div></div></div><div className="space-y-3"><section className="rounded-xl border border-line/80 bg-recess/70 p-3"><label className="label-mono mb-1.5 block text-2xs text-ink-low">ASSIGN TECHNICIAN</label><select data-testid="tech-select" value={job.techId ?? ""} onChange={e => void performAssignment(job.id, e.target.value, job.startBlock)} className="min-w-0 flex-1 rounded-md border border-line bg-recess px-1.5 py-1 text-2xs text-ink outline-none focus:border-chrome-400"><option value="" disabled>Unassigned</option>{technicians.map(t => <option key={t.id} value={t.id}>{t.name} — {t.van}</option>)}</select><span className="label-mono tnum shrink-0 text-2xs text-ink-low">{blockLabel(job.startBlock)}→{blockLabel(job.startBlock + job.spanBlocks)}</span></section><TimerControl job={job} /><JobMessageThread jobId={job.id} /><section className="rounded-xl border border-line/80 bg-recess/70 p-3"><label className="label-mono mb-1.5 block text-2xs text-ink-low">CUSTOMER NOTIFICATION</label><button type="button" data-testid="notify-eta" className="flex w-full items-center justify-center gap-1.5 rounded-md border border-line bg-recess px-2 py-1.5 text-2xs font-semibold text-ink-mid hover:border-chrome-400/50 hover:text-chrome-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-chrome-400" onClick={() => void (async () => { const tech = technicians.find(t => t.id === job.techId); const eta = tech?.lastKnownLocation && job.location ? travelMinutes(tech.lastKnownLocation, job.location) : 15; try { await authApi.sendEta(job.id, eta) } catch { /* best-effort: dispatch never blocks on SMS provider state */ } })()}><MessageSquare className="h-3 w-3" />NOTIFY CUSTOMER — ON MY WAY</button></section>{job.photos && job.photos.length > 0 && <section className="rounded-xl border border-line/80 bg-recess/70 p-3" data-testid="job-photos"><label className="label-mono mb-1.5 block text-2xs text-ink-low">FIELD EVIDENCE · {job.photos.length}</label><div className="grid grid-cols-2 gap-2">{job.photos.map(photo => <a key={photo.id} href={photo.url} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-lg border border-line bg-recess">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={photo.url} alt={`${photo.label} evidence`} className="h-20 w-full object-cover" loading="lazy" /><div className="label-mono truncate px-1.5 py-1 text-[10px] text-ink-low">{photo.label}</div></a>)}</div></section>}{job.linkedGroupId && <LinkedFragments job={job} />}</div></div>
    </motion.aside>
      )}
    </AnimatePresence>
  )
