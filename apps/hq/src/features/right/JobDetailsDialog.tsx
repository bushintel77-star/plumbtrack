...
function LinkedFragments({ job }: { job: { linkedGroupId?: string; id: string } }) {
  ...
  return (
    <section className="rounded-lg border border-white/10 bg-white/5 p-3" data-testid="linked-fragments">
...
export function JobDetailsDialog() {
  ...
  return (
    <aside className="fixed inset-y-12 right-3 z-40 flex w-[min(430px,calc(100vw-24px))] flex-col overflow-hidden rounded-xl border border-white/15 bg-slate-950/90 text-white shadow-2xl backdrop-blur-2xl" aria-label="Job details" data-testid={`inspector-${job.id}`}>
      <div className="shrink-0 border-b border-white/10 px-4 py-3"><button type="button" aria-label="Close details" data-testid="details-close" onClick={closeDetails} className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-md text-slate-400 hover:bg-white/10 hover:text-white focus-visible:ring-2 focus-visible:ring-blue-400"><X className="h-4 w-4" /></button><div className="pr-9"><h2 className="truncate text-base font-bold">{job.title}</h2><p className="label-mono tnum mt-1 text-2xs text-slate-400">{job.id.toUpperCase()} · {job.status} · {job.priority.toUpperCase()}</p><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs"><span className="font-semibold text-slate-100">{job.client}</span><span className="text-slate-400">{job.address}</span></div></div></div>
      <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto p-3"><div className="space-y-2.5"><section className="rounded-lg border border-white/10 bg-white/5 p-3"><div className="flex items-center justify-between"><label className="label-mono text-2xs text-slate-400">ASSIGNMENT</label><span className="label-mono tnum text-2xs text-slate-400">{blockLabel(job.startBlock)} → {blockLabel(job.startBlock + job.spanBlocks)}</span></div><select data-testid="tech-select" value={job.techId ?? ""} onChange={e => void performAssignment(job.id, e.target.value, job.startBlock)} className="mt-2 w-full rounded-md border border-white/10 bg-slate-900 px-2 py-2 text-xs text-white outline-none focus:border-blue-400"><option value="" disabled>Unassigned</option>{technicians.map(t => <option key={t.id} value={t.id}>{t.name} — {t.van}</option>)}</select></section><TimerControl job={job} />{job.linkedGroupId && <LinkedFragments job={job} />}</div></div>
    </aside>
  )
}
